...
class ConfluenceClient:
    ...
    def search_my_pages(self, email, api_token, months=12, max_results=200):
        ...
        for user_id in user_ids:
            user_cql = '(creator={} OR contributor={})'.format(_cql_escape(user_id), _cql_escape(user_id))
            cql_with_user_and_date = (
                'type=page AND {} AND lastModified>=endOfDay("-{}M") order by lastModified desc'
            ).format(user_cql, months)
            try:
                pages = self._search_paginated(auth, cql_with_user_and_date, expand, limit_per_page, max_results, cutoff=None)
                if pages:
                    return pages
            ...
            try:
                pages = self._search_paginated(auth, cql_with_user, expand, limit_per_page, max_results, cutoff=cutoff)
                if pages:
                    return pages
            except ConfluenceError as e:
                if e.status_code not in (400, 404):
        ...
        try:
            return self._search_paginated(auth, cql_current, expand, limit_per_page, max_results, cutoff=None)
        except ConfluenceError as e:
            if e.status_code not in (400, 404):
                raise
        cql_current_no_date = (
        ...
        return self._search_paginated(auth, cql_current_no_date, expand, limit_per_page, max_results, cutoff=cutoff)
    ...
    def search_my_pages_created(self, email, api_token, months=12, max_results=500):
        ...
        for user_id in user_ids:
            ...
            try:
                return self._search_paginated(auth, cql, expand, limit_per_page, max_results, cutoff=None)
            except ConfluenceError as e:
                if e.status_code not in (400, 404):
                    raise
        cql = 'type=page AND creator=currentUser() AND created>=endOfDay("-{}M") order by created desc'.format(months)
        try:
            return self._search_paginated(auth, cql, expand, limit_per_page, max_results, cutoff=None)
