...
INCLUDE_CHANGELOG = os.environ.get("JIRA_INCLUDE_CHANGELOG", "true").lower() not in (
    "0",
    "false",
    "no",
)
...
class JiraClient:
    ...
    def search_my_issues(self, email, api_token, months=12, max_results=500):
        ...
        jql_with_date = (
            f"(assignee = currentUser() OR reporter = currentUser()) AND updated >= -{months}m"
        )
    ...
    def _login_session(self, username, password):
        ...
        for path in ["/rest/auth/1/session", "/jira/rest/auth/1/session"]:
            ...
            if resp.status_code != 404:
                raise JiraError(
                    f"Jira login returned {resp.status_code}", status_code=resp.status_code
                )
        raise JiraError(
            "Jira session login not available (404). Your Jira may have API access disabled."
        )
        ...
        for path in ["/rest/api/2/myself", "/rest/api/3/myself"]:
            ...
            try:
                resp = self._session.get(
                    url, auth=auth, timeout=30, headers={"Accept": "application/json"}
                )
            except requests.exceptions.RequestException as e:
                raise JiraError(f"Could not reach Jira: {e!s}")
            if resp.status_code == 401:
                try:
                    self._login_session(email, api_token)
                    resp = self._session.get(
                        url, auth=None, timeout=30, headers={"Accept": "application/json"}
                    )
                except JiraError:
                    raise
            if resp.status_code == 404:
    ...
    def _search_classic(self, api_path, auth, jql, max_results):
        ...
        base_fields = "summary,project,status,issuetype,created,updated,resolutiondate,assignee,reporter,parent,labels,priority,components,comment"
        # customfield_10020 = sprint, customfield_10016 = story points (Jira Cloud)
        extra = list(EPIC_LINK_FIELDS) + ["customfield_10020", "customfield_10016"]
        ...
        try:
            while True:
                ...
                if resp.status_code != 200:
                    raise JiraError(
                        f"Jira returned {resp.status_code}", status_code=resp.status_code
                    )
                data = resp.json()
