...
class SlackClient:
    ...
    def users_conversations(
        self, token, types="public_channel,private_channel", limit=100, cursor=None
    ):
    ...
    def get_my_messages(self, token, months=12, max_messages=500, max_channels=40):
        ...
        for ch in channels:
            ...
            while True:
                try:
                    resp = self.conversations_history(
                        token, ch_id, oldest_ts, limit=200, cursor=cursor
                    )
                except SlackError:
                    break
                ...
                for m in msgs:
                    ...
                    reply_count = (
                        m.get("reply_count")
                        if isinstance(m.get("reply_count"), (int, float))
                        else None
                    )
                    out.append(
                        {
