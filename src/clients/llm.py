...
def answer_question(question, context, history=None, max_tokens=500):
    """Answer a question grounded ONLY in the provided context (the user's own data).

    history: optional list of {"role": "user"|"assistant", "content": str} for follow-ups.
    Returns the answer string, or raises LLMError.
    """
    if not (question or "").strip():
        return ""
    system = (
        "You are a helpful assistant answering questions about ONE person's own work "
        "activity across Jira, Gerrit, Confluence, and Slack. Answer ONLY using the DATA "
        "provided below. If the data does not contain the answer, say you don't have that "
        "information instead of guessing. Be concise and specific, and cite numbers from "
        "the data when relevant. This is for personal reflection, not ranking or judging."
        "\n\n=== DATA ===\n" + (context or "(no data)")
    )
    ...
    return _call_llm(messages, max_tokens=max_tokens)
